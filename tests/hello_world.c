...
ei_bool_t process_key(ei_event_t* event)
{
	if (event->type == ei_ev_keydown)
		if (event->param.key.key_sym == SDLK_ESCAPE || (event->param.key.key_sym == SDLK_w && ei_has_modifier(event->param.key.modifier_mask, ei_mod_ctrl_left))) {
			ei_app_quit_request();
			return EI_TRUE;
		}

        /* Pour ouvrir plusieurs fenêtres grâce à Ctrl + N */
	if (event->type == ei_ev_keydown && event->param.key.key_sym == SDLK_n && ei_has_modifier(event->param.key.modifier_mask, ei_mod_ctrl_left)) {
		ei_widget_t*	window;
		...
		ei_point_t	window_position		= {50, 30};
...
int ei_main(int argc, char** argv)
{
	...
	ei_place(button, &button_anchor, &button_x, &button_y, NULL, NULL, &button_rel_x, &button_rel_y, &button_rel_width, NULL);
	/*
	ei_widget_t *window2 = ei_widget_create("toplevel", window);
	ei_toplevel_configure(window2, &window_size, &window_color, &window_border_width,
				&window_title, &window_closable, &window_resizable, NULL);
	ei_place(window2, NULL, &(window_position.x), &(window_position.y), NULL, NULL, NULL, NULL, NULL, NULL);
	*/
